    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="navbar-ex-3">
            <div class="navbar-nav me-auto">
                <a class="nav-item nav-link active" href="javascript:void(0)">Menu Assign Setup</a>
            </div>
...
<div class="card">
    <h5 class="card-header">Menu Assign Table</h5>
    ...
    <div class="modal-dialog modal-xl modal-simple modal-dialog-centered modal-add-new-role">
        <div class="modal-content p-3 p-md-5">
            <div class="modal-body">
                ...
                <!-- Add role form -->
                <form id="addMenuForm" class="row g-4" onsubmit="return false">
                    @csrf
                
                    <!-- Role Selection (Multiple Select) -->
                    <div class="col-md-6">
                        <label class="form-label fw-bold" for="role_id">Role</label>
                        <select id="role_id" name="role_id[]" class="form-select" style="height: 200px;" multiple>
                            <option value="">Select Roles</option>
                        </select>
                        <small class="form-text text-muted">Hold Ctrl (Cmd on Mac) to select multiple roles.</small>
                    </div>
                
                    <!-- Menu Name (Checkboxes) -->
                    <div class="col-md-6">
                        <label class="form-label fw-bold" for="menu_id">Menu Name</label>
                        <div id="menu_id_container" class="border rounded p-3" style="max-height: 200px; overflow-y: auto;">
                    ...
                    <div class="col-12 text-center mt-4">
                        <button type="button" class="btn btn-primary me-2" onclick="addData()">
...
$.ajax({
    url: '/get-menus', // Update this URL as per your route
    type: 'GET',
    dataType: 'json',
    success: function(data) {
        var menuContainer = $('#menu_id_container');
        menuContainer.empty(); 
        
        menuContainer.append(`
            <div class="form-check mb-2">
                <input class="form-check-input" type="checkbox" id="select_all_menus">
                <label class="form-check-label fw-bold" for="select_all_menus">
                    Select All
                </label>
            </div>
        `);
        ...
        $('#select_all_menus').on('change', function() {
            $('.menu-checkbox').prop('checked', $(this).prop('checked'));
        });

        // If any individual checkbox is unchecked, uncheck the Select All
        $('#menu_id_container').on('change', '.menu-checkbox', function() {
            if ($('.menu-checkbox:checked').length === $('.menu-checkbox').length) {
                $('#select_all_menus').prop('checked', true);
            } else {
                $('#select_all_menus').prop('checked', false);
            }
        });
    },
    error: function() {
        alert('Failed to retrieve menu data.');
    }
});
    ...
    $.ajax({
        url: '/get-roles', // Adjust this URL to match your route
        type: 'GET',
        dataType: 'json',
        success: function(data) {
            var roleSelect = $('#role_id');
            roleSelect.empty(); // Clear existing options
            roleSelect.append('<option value="">Select Roles</option>');
